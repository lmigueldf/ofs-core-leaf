...
exports = Class(function () {
  ...
  this.enable = function () {
    ...
    this._handleMove = $.onEvent(document, device.events.move, this, 'handleMouse', eventTypes.MOVE);
      !device.isMobile && (device.events.move == 'touchmove') && $.onEvent(document, 'mousemove', this, 'handleMouse', eventTypes.MOVE);
    this._handleSelect = $.onEvent(document, device.events.end, this, 'handleMouse', eventTypes.SELECT);
      !device.isMobile && (device.events.end == 'touchend') && $.onEvent(document, 'mouseup', this, 'handleMouse', eventTypes.SELECT);
  ...
  this._addElEvents = function () {
    ...
    if (!device.useDOM) {
      ...
      !device.isMobile && (device.events.start == 'touchstart') && this._elEvents.push($.onEvent(el, 'mousedown', this, 'handleMouse', eventTypes.START));
    }
  ...
  this.handleMouse = function (type, evt) {
    ...
    if (isMobileBrowser) {
     ...
     if (allowIOSScroll && type === eventTypes.SCROLL) {
      ...
      if (!allowIOSScroll) {
        $.stopEvent(evt);
          if(evt.type && evt.type != 'touchstart' && evt.type != 'mousewheel' && evt.type != 'touchmove') (evt.returnValue = false);
      }
